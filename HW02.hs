...
countColors = flip map colors . (\code col -> length $ filter (==col) code)
...
getMove secret guess = Move guess exactM (allM - exactM)
  where
...
isConsistent (Move guess exact matches) code = (getMove code guess) == (Move guess exact matches)
